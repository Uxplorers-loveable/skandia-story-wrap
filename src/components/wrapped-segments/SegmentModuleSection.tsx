          ...
          return (
            ...
            >
              ...
              <div className="text-center mb-10">
                {module.headline && (
                  <div className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-4 py-2 mb-4 rounded-full">
              ...
              {module.statCards && module.statCards.length > 0 && (
                <div className={`grid gap-4 mb-8 ${
                  ...
                  module.statCards.length === 4 ? 'md:grid-cols-2 lg:grid-cols-4' : 'md:grid-cols-2'
                }`}>
                  {module.statCards.map((stat, statIndex) => {
                    ...
                    return (
                      <div 
                        key={statIndex}
                        className={`rounded-2xl p-6 border transition-all duration-500 hover:scale-[1.02] hover:shadow-lg ${
                          stat.highlight 
                      ...
                      >
                        {StatIcon && (
                          <div className={`p-3 rounded-xl w-fit mb-4 ${stat.highlight ? 'bg-primary-foreground/20' : 'bg-primary/10'}`}>
                        ...
                        <p className={`text-3xl md:text-4xl font-bold mb-1 ${stat.highlight ? 'text-primary-foreground' : 'text-foreground'}`}>
                          ...
                          {stat.suffix && (
                            <span className={`text-lg md:text-xl ml-1 ${stat.highlight ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
                              {stat.suffix}
                            </span>
                          )}
                        </p>
                        ...
                        {stat.description && (
                          <p className={`text-xs mt-2 ${stat.highlight ? 'text-primary-foreground/60' : 'text-muted-foreground'}`}>
                            {stat.description}
                          </p>
                        )}
                      </div>
                ...
                <div className="space-y-4">
                  {module.cards.map((card, cardIndex) => {
                    ...
                    return (
                      <div 
                        key={cardIndex}
                        className={`bg-muted rounded-2xl p-6 border border-border transition-all duration-500 hover:shadow-lg ${
                          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                        }`}
              ...
              {module.closingMessage && (
                <div 
                  className={`mt-8 bg-muted border border-border rounded-2xl p-6 text-center transition-all duration-500 ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                  }`}
              ...
              {module.ctaLabel && (
                ...
                >
                  <div className="bg-muted border border-border rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-4 md:gap-6">
                    ...
                    <Button 
                      size="lg" 
                      className="bg-primary text-primary-foreground hover:bg-primary/90 px-8 shadow-lg transition-all shrink-0"
                    >
                      {module.ctaLabel}
