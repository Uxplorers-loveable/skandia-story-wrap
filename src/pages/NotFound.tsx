  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden bg-background">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div 
          className={`absolute top-1/4 left-[10%] w-32 h-32 rounded-full bg-primary/10 blur-3xl transition-all duration-1000 delay-300 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}
        />
        <div 
          className={`absolute bottom-1/4 right-[15%] w-48 h-48 rounded-full bg-primary/5 blur-3xl transition-all duration-1000 delay-500 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}
        />
        <div 
          className={`absolute top-1/2 left-1/4 w-24 h-24 rounded-full bg-secondary/5 blur-2xl transition-all duration-1000 delay-700 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}
        />
      </div>

      {/* Animated path lines - metaphor for exploration */}
      <svg 
        className={`absolute inset-0 w-full h-full pointer-events-none transition-opacity duration-1000 delay-500 ${visible ? 'opacity-100' : 'opacity-0'}`}
        xmlns="http://www.w3.org/2000/svg"
      >
        <path 
          d="M-50 400 Q 200 300 400 350 T 800 300 T 1200 350 T 1600 300" 
        ...
        />
        <path 
          d="M-50 500 Q 250 400 500 450 T 1000 400 T 1500 450" 
          stroke="hsl(var(--primary))" 
          strokeWidth="0.5" 
          fill="none" 
          opacity="0.08"
      ...
      <div className={`relative z-10 text-center px-6 max-w-2xl transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className={`flex flex-col sm:flex-row items-center justify-center gap-4 mb-12 transition-all duration-1000 delay-600 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <Button 
            ...
            onClick={() => window.location.href = 'https://skandia.com.co'}
          >
          ...
          <Button 
            variant="outline"
            size="lg"
            className="border-secondary text-secondary hover:bg-secondary hover:text-secondary-foreground px-8 py-6 text-base font-medium"
            onClick={() => window.location.href = 'https://skandia.com.co/contacto'}
          >
